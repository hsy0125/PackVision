...
{ /*
    ...
    */
    ...
    public class MatchAlgorithm : InspAlgorithm
    {
        ...
        public OpenCvSharp.Size ExtSize { get; set; } = new OpenCvSharp.Size(100, 100);
        ...
        public bool MatchTemplateSingle(Mat image, Point leftTopPos)
        {
            ...
            Debug.WriteLine($"[MatchAlgorithm] 최적 매칭 위치: {maxLoc}, 신뢰도: {maxScore:F2}");
        ...
        public override bool DoInspect()
        {
            ...
            if (_templateImages.Count <= 0)
            {
                Debug.WriteLine("MatchAlgorithm: 티칭 이미지 없음");
                return false;
            }

            if (_templateImages[0].Type() == MatType.CV_8UC3)
            {
                Debug.WriteLine("MatchAlgorithm: 티칭은 그레이스케일만 지원");
                return false;
            }
            ...
            Mat srcImage = _srcImage;

            int padX = Math.Max(0, ExtSize.Width);
            int padY = Math.Max(0, ExtSize.Height);
            Rect extArea = new Rect(
                InspRect.X - padX,
                InspRect.Y - padY,
                InspRect.Width + 2 * padX,
                InspRect.Height + 2 * padY);
            ...
            if (extArea.Width <= 0 || extArea.Height <= 0)
                return false;
